                                ...
                                <tbody>
                                    ...
                                    @foreach ($usercompanies as $companies)
                                        <tr>
                                            <td>{{ $count++ }}</td>
                                            <td><a
                                                    href="{{ route('companies.show', $companies->id) }}">{{ $companies->name }}</a>
                                            </td>
                                            <td>{{ $companies->email }}</td>
                                            <td>{{ $companies->checkout->sum('total') }}</td>
                                            <td>
                                                @if ($companies->user_type == '2')
                                                    <a href=""class="text-muted">{{ trans('lang.company') }}</a>
                                                @elseif ($companies->user_type == '3')
                                                    <a href="" class="text-muted">{{ trans('lang.user') }}</a>
                                                @else
                                                @endif
                                            </td>
                                            <td>
                                                @if ($companies->status == '1')
                                                    <a href=""class="text-muted">{{ trans('lang.active') }}</a>
                                                @elseif ($companies->status == '0')
                                                    <a href="" class="text-muted">{{ trans('lang.inactive') }}</a>
                                                @else
                                                @endif
                                            </td>
                                            <td>{{ date('d-m-Y', strtotime($companies->created_at)) }}</td>
                                            <td>
                                                <a href="{{ route('companies.show', $companies->id) }}" title=""
